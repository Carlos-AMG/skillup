...
const generateOfferDetailsHTML = (offerDetails) => {
  ...
  `;
    ...
    const companyActions = state.userType === "companies" ? `
      <div class="py-2 inline-block mb-3 ">
        ...
        <a href="/companies/edit-offer/${state.currentFilterJobCourse}/${offerDetails.id}" class="px-5 bg-yellow-500 text-white uppercase hover:bg-yellow-700 font-bold py-3 cursor-pointer rounded-md mx-3">Update </a>
        <a href="/companies/skillers/${state.currentFilterJobCourse}/${offerDetails.id}" class="px-5 bg-blue-500 text-white uppercase hover:bg-blue-700 font-bold py-3 cursor-pointer rounded-md mx-3 ">Skillers</a>
      </div>
    ` : "";
